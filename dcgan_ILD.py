...
def identity_block(X, f, filters, stage, block):
    ...
    X =  Add()([X_shortcut, X])  # equivalent to added = keras.layers.add([x1, x2])
    X = Activation('relu')(X)
...
def generator_model():
    ...
    model.add(Dense(128*64*64))
    model.add(BatchNormalization())
    model.add(Activation('tanh'))
    # model.add(Reshape((7, 7, 128), input_shape=(128*7*7,)))
    model.add(Reshape((64, 64, 128), input_shape=(128*64*64,)))
...
def train(BATCH_SIZE):
    ...
    path1 = 'H:/lung/CNN/posterior/images'    #path of folder of images    
    path2 = 'H:/git/keras-dcgan/resized_256'  #path of folder to save images    
    ...
    d_optim = adam(lr=0.0001, momentum=0.9, nesterov=True)
    g_optim = adam(lr=0.0001, momentum=0.9, nesterov=True)
    ...
    for epoch in range(10):
        ...
        for index in range(int(X_train.shape[0]/BATCH_SIZE)):
            ...
            image_batch = image_batch.reshape(128,img_rows,img_cols,1) 
            print(shape(image_batch))
            generated_images = g.predict(noise, verbose=0)
            print(shape(generated_images))
            if index % 20 == 0:
